...
import { baseUrl,baseStoreURL,mediaThumbProductImage } from '@/repositories/Repository';
...
export function CartProductThumbnail(product) {
    let view;

    if (product.product_main_image) {
        view = (
            <Link href={`${baseStoreURL}/${product.category_name.replace(/ /g,"-")}/${product.product_sku}/${product.url_slug}.html`}>
                <a>
                    <LazyLoad>
                        <img className="img-fluid mx-auto d-block image"
                            src={`${mediaThumbProductImage}/${product.product_main_image}`}
                            alt={product.title}
                        />
                    </LazyLoad>
                </a>
        ...
        view = (
            <Link href={`${baseStoreURL}/${product.category_name.replace(/ /g,"-")}/${product.product_sku}/${product.url_slug}.html`}>
                <a>
                    <LazyLoad>
                        <img className="img-fluid mx-auto d-block image" src={`${baseStoreURL}/static/img/not-found.jpg`} alt={product.title} />
                    </LazyLoad>
                </a>
            </Link>
        );
